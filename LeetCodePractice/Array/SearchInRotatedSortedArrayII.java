...
..., nums[k-1]] (0-indexed). For example, [0,1,2,4,4,4,5,6,6,7] might be rotated at pivot index
5 and become [4,5,6,6,7,0,1,2,4,4].Given the array nums after the rotation and an integer target, 
return true if target is in nums, or false if it is not in nums.You must decrease the overall 
     ...
     while(low<=high)
     {
      ...
      if(arr[mid]==target)
      {
       ...
       if(arr[low]<arr[mid])
       {
         ...
         {
          low++;   //beacuse of this time become O(n)
         }
       else
       {
